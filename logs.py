...
logging.warning("Esto es un aviso, warning")
...
class TaskManager:
    ...
    def add_task(self, name: str, description: str):
        ...
        if name not in self.tasks:
            ...
            logging.info(f"Tarea aÃ±adida: {name}.")
        else:
            logging.warning(
                f"Se ha intentado aÃ±adir una tarea que ya existe: {name}.")
        logging.debug(f"NÃºmero de tareas: {len(self.tasks)}")
        end_time = time.time()
    ...
    def _print_time(self, start_time, end_time):
        logging.debug(
            f"Tiempo de ejecuciÃ³n: {end_time - start_time:.6f} segundos.")
